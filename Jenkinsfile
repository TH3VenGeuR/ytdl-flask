    ...
    stages {
        ...
        stage('Deploy') {
            steps {
				...
				script {
                    try{
						def old_container_id_list = sh "docker --host $DOCKER_HOST ps -a | grep ilyatrof/ytdl-flask | awk '{ print \$1 }'"
                    ...
                    try{
						def old_images_id_list = sh "docker --host $DOCKER_HOST images | grep ilyatrof/ytdl-flask | awk '{ print \$3 }'"
